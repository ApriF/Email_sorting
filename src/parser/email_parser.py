    ...
    if msg.is_multipart():
        ...
        for part in msg.walk():
            ...
            if content_type == "text/plain":
                text_parts.append(get_decoded_payload(part))

            elif content_type == "text/html":
                html_parts.append(get_decoded_payload(part))
    else:
        # the email is just a single block of text with no attachments
        content_type = msg.get_content_type()
        if content_type == "text/plain":
            text_parts.append(get_decoded_payload(msg))
        elif content_type == "text/html":
            html_parts.append(get_decoded_payload(msg))
